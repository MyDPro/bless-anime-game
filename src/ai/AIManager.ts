...
export class AIManager {
  ...
  private enemies: { id: string; model: THREE.Object3D; health: number; speed: number; power: number; damage: number; type: string }[] = [];
...
private checkCollision(position: THREE.Vector3, size: { width: number; depth: number }): boolean {
  ...
  getEnemies(): { id: string; model: THREE.Object3D; health: number; power: number; speed: number; damage: number; type: string }[] {
    return this.enemies;
  }
