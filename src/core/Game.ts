...
export class Game extends EventEmitter {
    ...
    dirLight.shadow.mapSize.width = 2048; // Daha büyük harita için artır
    dirLight.shadow.mapSize.height = 2048;
    dirLight.shadow.camera.near = 0.5;
    dirLight.shadow.camera.far = 500;
    dirLight.shadow.camera.left = -250; // 500x500 harita
    ...
    const platform = new THREE.Mesh(
        new THREE.BoxGeometry(500, 0.5, 500), // 50x50 -> 500x500
    ...
    cityData.buildings.forEach(data => {
        ...
        if (model) {
            model.position.set(Math.random() * 500 - 250, 0, Math.random() * 500 - 250); // Rastgele [-250, 250]
            model.scale.setScalar(3); // Daha büyük harita için ölçek artır
