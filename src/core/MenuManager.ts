...
export class MenuManager {
    ...
    constructor(modelsLoader: ModelsLoader) {
        ...
        this.menus = new Map();
        this.sharedRenderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
    ...
    private async setupCharacterPreview(characterId: string): Promise<void> {
        ...
        if (canvas.clientWidth === 0 || canvas.clientHeight === 0) {
            console.warn(`Canvas boyutları sıfır, varsayılan boyutlar uygulanıyor: ${characterId}`);
        ...
        this.sharedRenderer.setSize(canvas.clientWidth, canvas.clientHeight);
        this.sharedRenderer.setPixelRatio(window.devicePixelRatio);
        camera.aspect = canvas.clientWidth / canvas.clientHeight;
        camera.updateProjectionMatrix();

        canvas.appendChild(this.sharedRenderer.domElement);
        ...
        this.characterPreviews.set(characterId, { scene, camera, renderer: this.sharedRenderer });
        ...
        console.log(`Karakter modeli alındı: ${characterId}`, gltf);
        ...
        scene.add(model);
        console.log(`Model sahneye eklendi: ${characterId}`, model);
